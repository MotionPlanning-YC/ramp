...
Planner::Planner() : resolutionRate_(1.f / 10.f), ob_dists_timer_dur_(0.1), generation_(0), i_rt(1), goalThreshold_(0.4), num_ops_(6), D_(1.5f), 
 ...
 delta_t_switch_(0.1), stop_(false), moving_on_coll_(false), log_enter_exit_(false), log_switching_(false), log_all_(false)
{
...
void Planner::sensingCycleCallback(const ramp_msgs::ObstacleList& msg)
{
  ...
  if(ob_trajectory_.size() > 0)
  {
    ...
    // Get direction and distance to closest obstacle
...
const double Planner::updateCurvePos(const RampTrajectory& traj, const ros::Duration& d) const 
{
  ...
  if(curve.u_0 < 0.00001) 
  {
    ...
    if(log_all_ && index < curve.u_values.size())
    {
...
void Planner::buildEvaluationRequest(const RampTrajectory& trajec, ramp_msgs::EvaluationRequest& result, bool full) const
{
  ...
  double nec_theta;
  if(trajec.msg_.i_knotPoints.size() < 1 || trajec.msg_.trajectory.points.size() < trajec.msg_.i_knotPoints[1])
  {
    nec_theta = trajec.msg_.trajectory.points[ trajec.msg_.trajectory.points.size()-1 ].positions[2];
  }
  else
  {
    nec_theta = trajec.transitionTraj_.trajectory.points.size() > 0 ?
  ...
  /*
   * Previous way of setting nec_theta, but this crashed quite ofen
   * Fix this bug 
...
void Planner::obICCallback(const ros::TimerEvent& e)
{
  ...
  for(uint8_t i=0;i<ob_trajectory_.size();i++)
  {
    double dist = ob_trajectory_.at(i).msg_.trajectory.points.size() > 0 ? 
      utility_.positionDistance(ob_trajectory_.at(i).msg_.trajectory.points.at(0).positions, latestUpdate_.msg_.positions) :
      9999;
...
void Planner::computeFullSwitch(const RampTrajectory& from, const RampTrajectory& to, const double& t_start, RampTrajectory& result)
{
  ...
  if(to.msg_.trajectory.points.size() == 1)
  {
    result = to;
    return;
  }
  ...
  else if(trajec.msg_.trajectory.points.size() == 1)
  {
    trajec = to;
  }
...
trajectory_msgs::JointTrajectoryPoint Planner::prepareForTestCase()
{
  ...
  // Send blank trajectory to robot
  RampTrajectory blank;
  h_control_->send(blank.msg_);
